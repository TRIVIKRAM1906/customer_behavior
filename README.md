...
2. **Initial Exploration:** Used `.info()` and `.describe()` for data structure and summary statistics.  
3. **Missing Data Handling:** Imputed missing review ratings using the median rating per product category.  
4. **Column Standardization:** Converted column names to `snake_case` format.  
5. **Feature Engineering:**  
   - Created `age_group` by binning customer ages.  
   - Derived `purchase_frequency_days` from purchase timestamps.  
6. **Data Consistency Check:** Compared `discount_applied` vs. `promo_code_used` and dropped redundant columns.  
7. **Database Integration:** Loaded the cleaned DataFrame into **PostgreSQL** for SQL-based analysis.
...
1. Clone this repository or download the `.pbix` and `.ipynb` files.  
...
Express shipping correlated with higher order values.
